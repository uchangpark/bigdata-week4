...
inflation = pd.Series((2.2, 3.4, 2.8, 1.6, 2.3, 2.7,  
        3.4, 3.2, 2.8, 3.8, -0.4, 1.6, 3.2, 2.1, 1.5, 1.5))
print('*inflation=>', inflation); 
print('*len(inflation)=>', len(inflation))
print('*inflation.values=>', inflation.values)
print('*inflation.index=>', inflation.index)
print('*inflation.index.values=>', inflation.index.values)
...
print('*len(inflation)=>', len(inflation))
inflation = pd.Series((2.2, 3.4, 2.8, 1.6, 2.3, 2.7, 
          3.4, 3.2, 2.8, 3.8, -0.4, 1.6, 3.2, 2.1, 1.6, 1.5))
inflation.index = pd.Index(range(1999, 2015))
inflation[2015] = np.nan # by Uchang
print('*inflation=>',inflation);  
print('*len(inflation)=>', len(inflation))
inflation.index.name = "Year" 
inflation.name = "%"
print('*inflation=>', inflation);
print('*inflation.head', inflation.head())
print('*inflation.tail', inflation.tail)
#---------------------------------------


# In[10]:
import pandas as pd
# 현재 작업 디렉토리를 살펴보고 작업 디렉토리를 변경한다.....
import os
print (os.getcwd())
os.chdir('C:/Google02/LECTURE2020/5-BigData/10-ipynb1-BigData-org')

alco2009 = pd.read_csv("niaaa-report2009.csv", index_col="State")
#alco2009 = pd.read_csv("niaaa-report2009.csv")
print('*alco2009=>', alco2009); 
print('*len(alco2009)=>', len(alco2009))
print('*alco2009["Wine"].head()=>', alco2009["Wine"].head())
print('*alco2009.Beer.tail()=>', alco2009.Beer.tail())
alco2009["Total"] = 0  # 모두 0으로 변경 
print('*alco2009.head()=>', alco2009.head())

# In[13]:
# unit32 데이터모양바꾸기
print('*alco2009.columns.values=>', alco2009.columns.values)
print('*alco2009.index.values=>', alco2009.index.values)
print(alco2009.loc["Nebraska"])
print('*Samoa" in alco2009.index=>', "Samoa" in alco2009.index)
...
print('*tall_alco.head(10)=>', tall_alco.head(10))

wide_alco = alco.unstack()
print('*wide_alco.head(10)=>', wide_alco.head())
#---------------------------------------
alco = pd.read_csv("niaaa-report.csv")
alcopivot=alco.pivot("Year", "State", "Wine")
print(alcopivot)

# In[27]:
# unit 33 데이터 누락 다루기
print('*nan_alco=>', nan_alco)
print('*nan_alco.dropna(how="all")=>', nan_alco.dropna(how="all")) # 모두 nan일 경우
print('*nan_alco.dropna(how="all", axis=1)=>', nan_alco.dropna(how="all", axis=1)) # axis=0 행, axis=1 열
print('*nan_alco.dropna()=>', nan_alco.dropna())
print('*nan_alco.isnull()=>', nan_alco.isnull())
print('*nan_alco.notnull()=>', nan_alco.notnull())
#---------------------------------------
sp = nan_alco['Spirits'] # 결측값이 있는 열을 선택했다.
print('*sp=>', sp)
clean = sp.notnull() 
print('*sp.notnull()=>', clean) 
print('*nan_alco=>', nan_alco)
print('*nan_alco.fillna(0)=>', nan_alco.fillna(0))
...
df.head()
df = pd.merge(alco2009, population, left_index=True, right_index=True) # join 연산
df.head()
df1=population.join(alco2009) # join 연산 
df2=population.join(alco2009).tail(10) 
df3=pd.concat([alco2009, population], axis=1).tail() # join 연산 끝 5개.
#---------------------------------------
...
# unit 35 데이터 정렬 
#---------------------------------------
p1=population.sort_index().head() # 정렬하여 head() 적용
population.sort_values("Population").head()
# 정렬이 잘되지 않았다? population 값을 문자료 인식한다.
#population.applymap(atof)
population['Population'] = population['Population'].str.replace(',', '').astype(int)
p2=population.sort_index().head() # 정렬하여 head() 적용
population.sort_values("Population").head()
...
#---------------------------------------
import pickle # pd 데이터 저장 및 로드
with open('alco.pickle', 'wb') as oFile:
  pickle.dump(alco, oFile)
with open('alco.pickle', 'rb') as oFile:
  alco1=pickle.load(oFile)
  print(alco1)
